...
class MyCollectiveAllReduce(CollectiveAllReduce):
  ...
  def reduce_implementation(self, reduce_op, per_replica_value, destinations):
    print("xxxxxx cross_device_ops.py:1055:reduce_implementation")
    ...
    if (isinstance(all_reduced, value_lib.Mirrored) and
      ...
      print("xxxxxx cross_device_ops.py:1063:reduce_implementation")
      return all_reduced
    ...
    if not isinstance(all_reduced, value_lib.Mirrored):
      ...
      print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno)
    ...
    with ops.control_dependencies(all_reduced.values):
      frameinfo = getframeinfo(currentframe())
      print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno, "len(devices)", len(devices))
      for d in devices:
        frameinfo = getframeinfo(currentframe())
        print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno)
        with ops.device(d):
...
def my_collective_reduce(input, group_size, group_key, instance_key, merge_op, final_op, subdiv_offsets, wait_for=[], communication_hint="auto", name=None):
  ...
  frameinfo = getframeinfo(currentframe())
  print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno, "communication_hint ", communication_hint)
  ...
  # my_new_wrapper_around_byteps_push_pull should be a wrapper around
  # byteps_push_pull, and has the same function signature as the original.
  # communication_hint=communication_hint should be ignored. call
  # byteps_push_pull anyway.
...
class BytepsCrossDeviceOps(tf_cross_device_ops.CrossDeviceOps):
  def __init__(self):
    ...
    # x2682 corresponding to class ReductionToOneDevice
  def reduce_implementation(self, reduce_op, per_replica_value, destinations):
    frameinfo = getframeinfo(currentframe())
    print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno)
    ...
    #     frameinfo = getframeinfo(currentframe())
    #     print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno)
    #     reduced = _push_pull(per_replica_value)
    #     # strategy.run(_push_pull, args=(per_replica_value,))
    #     frameinfo = getframeinfo(currentframe())
    #     print("xxxxxx ", frameinfo.filename, ":", frameinfo.lineno)
    return reduced
