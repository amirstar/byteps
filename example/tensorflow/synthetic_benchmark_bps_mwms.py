...
os.environ['TF_CONFIG'] = json.dumps({
    ...
    'task': {'type': 'worker', 'index': bps.rank()}
})

#print("my bps rank ", bps.rank())
...
strategy = bps.distribute.MultiWorkerMirroredStrategy()
# os.system("tmux clear-history; clear")
#print("=============================== created strategy" )
with strategy.scope():
    # Set up standard model.
    # print("=============================== going to create  model rank", bps.rank() )
    model = getattr(applications, args.model)(weights=None)
    # print("=============================== created model rank", bps.rank() )
...
#        tape = bps.DistributedGradientTape(tape, compression=compression)
        ...
        # do I need to compute mean loss?
        return loss

#    per_example_loss = strategy.run(step_fn, args=())
    per_example_loss = strategy.experimental_run_v2(step_fn, args=())
    mean_loss = strategy.reduce(tf.distribute.ReduceOp.MEAN, per_example_loss, axis=0)
    # print("============== done one step")
#    return mean_loss
